    ...
    try:
        ...
        for x in socre_table_sort:
            ...
            for token in globals()[x[0]]:
                token_cnt_list.append(token[1])
            url_text= hp_all[hp_all['name'] == x[0]]['주소'].values[0]
            img_text=hp_all[hp_all['name'] == x[0]]['사진'].values[0]
            name_dept= hp_all[hp_all['name'] == x[0]]['name_dept'].values[0]
            
            if sort_count < 6:
                hp_list.append(rank_list + (str(sort_count) + "순위:" +  x[0]))
                recom_list.append("-추천점수: " + str(x[1].round(2)) + "\n-" +  name_dept + " -검색어갯수:" + str(sum(token_cnt_list)))               
...
def hp_rank_output_facebook(message):
    try:
        ...
        for x in socre_table_sort:
            ...
            url_text= hp_all[hp_all['name'] == x[0]]['주소'].values[0]
            name_dept= hp_all[hp_all['name'] == x[0]]['name_dept'].values[0]
            address= hp_all[hp_all['name'] == x[0]]['address'].values[0]
            
            if sort_count < 6:
                rank_list = rank_list + ("이름" +  x[0] + "\n-진료과: " +  name_dept + "\n-주소: " +  address + "\n-검색트래픽:" + str(sum(token_cnt_list)) + "\n" +  url_text + "\n") 
            sort_count += 1
    except:
        search_list = '관련 검색어가 없습니다'
        rank_list = '재검색해주시기 바랍니다.'
